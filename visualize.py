...
def _generate_facet_plot(all_data, columns):
    fig = px.scatter(
    all_data,
    x=columns[0],
    y=columns[1],
    color=columns[2],  # Optional: differentiate points by color
    facet_col=columns[2],  # Create facets based on the categorical column
    title="Facet Grid: Numerical vs Numerical by Category",
)
...
def _generate_scatter3d_plot(all_data, columns):
    """Generate a scatter plot for column pair plot.
    ...
    fig = px.scatter_3d(all_data, x=columns[0], y=columns[1], z=columns[2], 
                    title="3D Scatter Plot of 3 Numerical Columns",
                    labels={'Column1': columns[0], 
                            'Column2': columns[1], 
                            'Column3': columns[2]})
...
def _generate_bubble3d_plot(all_data,columns):
    fig = px.scatter(
    all_data,
    x=columns[0],           # x-axis
    y=columns[1],          # y-axis
    size=columns[2],            # Bubble size
    color=columns[2],           # Color based on bubble size
    title=f"Bubble Chart: {columns[0]}, {columns[1]}, {columns[2]}"
)
...
def _generate_pairplot_plot(all_data,columns):
    
    fig = px.scatter_matrix(
    all_data,
    dimensions=[columns[0], columns[1], columns[2]],
    color=columns[3],  
    title=f"PairPlot: {columns[0]}, {columns[1]}, {columns[2]}, {columns[3]}",
...
def _generate_sunburst_plot(all_data,columns):
    fig = px.sunburst(
    all_data,
    path=[f"{columns[0]}", f"{columns[1]}", f"{columns[2]}", f"{columns[3]}"],  # Order of hierarchy
    title="Sunburst Plot: {columns[0]}, {columns[1]}, {columns[2]}, {columns[3]}",
...
def _generate_faceted4d_plot(all_data,columns):
    fig = px.scatter(
    all_data,
    x=f"{columns[0]}",          # First numerical column on the x-axis
    y=f"{columns[1]}",          # Second numerical column on the y-axis
    size=f"{columns[1]}",       # Bubble size based on the second numerical column
    color=f"{columns[3]}",       # Color bubbles by the second categorical column
    facet_col=f"{columns[2]}",   # Facet the plots by the first categorical column
    title="Faceted Plot: {columns[0]}, {columns[1]}, {columns[2]}, {columns[3]}",
)
    ...
    legend=dict(title=f"{columns[3]} Legend"),
        font={'size': PlotConfig.FONT_SIZE},
    )
    return fig
...
def _generate_facet_plot(all_data, columns):
    ...
    fig = px.scatter(all_data,
        ...
        color=columns[2],  # Optional: differentiate points by color
        facet_col=columns[2],  # Create facets based on the categorical column
        title=f"Facet Grid: {columns[0]}, {columns[1]}, {columns[2]}"
    )
    #title = f" Data for columns '{columns[0]}' , '{columns[1]}' and {columns[2]}"
...
@set_plotly_config
def get_column_triple_plot(real_data, synthetic_data, column_names, plot_type=None):
    """Return a plot of the real and synthetic data for a given column pair.
    ...
    if plot_type not in ['box', 'heatmap', 'scatter', None,'scatter3d','facet']:
        raise ValueError(
            f"Invalid plot_type '{plot_type}'. Please use one of "
            "['box', 'heatmap', 'scatter', None]."
        )
    ...
    if plot_type is None:
        ...
        for column_name in column_names:
            ...
            dtype = column.dropna().infer_objects().dtype.kind
            if dtype in ('i', 'f') or is_datetime(column):
                plot_type.append('bubble3d')
            else:
                plot_type.append('scatter3d')
    ...
    if plot_type == 'scatter3d':
        return _generate_scatter3d_plot(all_data, column_names)
    elif plot_type == 'facet':
        return _generate_facet_plot(all_data, column_names)
    elif plot_type == 'bubble3d':
        return _generate_bubble3d_plot(all_data, column_names)
    


@set_plotly_config
def get_column_quad_plot(real_data, synthetic_data, column_names, plot_type=None):
    """Return a plot of the real and synthetic data for a given column pair.
    ...
    if plot_type not in ['box', 'heatmap', 'scatter', None,'scatter3d','facet','faceted4d','sunburst4d','pairplot','4d']:
        raise ValueError(
            f"Invalid plot_type '{plot_type}'. Please use one of "
            "['box', 'heatmap', 'scatter', None]."
        )
    ...
    if plot_type is None:
        ...
        for column_name in column_names:
            ...
            dtype = column.dropna().infer_objects().dtype.kind
            if dtype in ('i', 'f') or is_datetime(column):
                plot_type.append('faceted4d')
            else:
                plot_type.append('sunburst4d')
    ...
    if plot_type == 'faceted4d':
        return _generate_faceted4d_plot(all_data, column_names)
    elif plot_type == 'sunburst4d':
        return _generate_sunburst_plot(all_data, column_names)
    elif plot_type == 'pairplot':
        return _generate_pairplot_plot(all_data, column_names)
